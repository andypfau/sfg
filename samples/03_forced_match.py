...
network = SFG(group_name_separator='.')
...
# create plot
g = network.plot()
g.render(outfile='output/forced_match.pdf', view=True, cleanup=True)
for i,g in enumerate(network.plot_loops()):
    g.render(outfile=f'output/forced_match_loop{i}.pdf', view=True, cleanup=True)
for i,g in enumerate(network.plot_paths('Source.b', 'Source.a')):
    g.render(outfile=f'output/forced_match_s11-{i}.pdf', view=True, cleanup=True)
